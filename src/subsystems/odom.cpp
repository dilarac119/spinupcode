...
static const double WHEEL_RADIUS = 1.375;
...
void movePIDOdom(float leftTarget, float rightTarget, int ms, float maxV) {
    float currentLeftTravel = (leftEncoder.controllerGet() * (2.75 * M_PI)) / 36000.0f;   // [in]
    float currentRightTravel = (rightEncoder.controllerGet() * (2.75 * M_PI)) / 36000.0f; // [in]
    ...
    while (timer < ms) { // Within time limit, increment PID loop
        ...
        currentLeftTravel = (leftEncoder.controllerGet() * (2.75 * M_PI)) / 36000.0f;
        currentRightTravel = (rightEncoder.controllerGet() * (2.75 * M_PI)) / 36000.0f;
...
void gyroPID(float degree, bool CW, int ms) {
    ...
    float integral = 0;
    while (timer < ms) {
        float sensorVal = imuVal(imu1.controllerGet(), imu2.controllerGet()) - taredRotation;
        float error = degree - sensorVal;
        float derivative = error - prevError;
        prevError = error;
        integral += error;
        float power = (.5 * error) + (0 * integral) + (0 * derivative);
        if (CW) {
